...
export default function GamePage({ onLogout, loggedIn = true, onLogin }: Props) {
  ...
  const gs = useRef({
    shooter: { x: CW / 2 - 48, y: CH - 110, w: 96, h: 96, speed: 10 },
    bullets: [] as Bullet[],
    zombies: [] as Zombie[],
  ...
  async function startGame() {
    ...
    const s = gs.current;
    s.shooter = { x: CW / 2 - 48, y: CH - 110, w: 96, h: 96, speed: 10 };
  ...
  function loop() {
    ...
    // Zombies
    const spawnChance = 0.038 + s.diffMult * 0.012;
    if (Math.random() < spawnChance) {
      const sz = 72 + Math.random() * 20;
      const speed = (2.8 + Math.random() * 1.8) * s.diffMult;
